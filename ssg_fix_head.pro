...
pro ssg_fix_head, indir, outdir
  ...
  for i=0,nf-1 do begin
     ...
     if err ne 0 then begin
        ...
        if count eq 0 then $
          message, 'No PROPID keyword.'
        date = sxpar(hdr, 'DATE', count=count)
        if count eq 0 then $
          message, 'No DATE keyword.'
        ...
        datearr=strsplit(date,'-T:',/extract)
        dir_array = strsplit(indir, '/', /extract)

        ;; YEAR.  This assumes that the file was written 
        ...
        if year_from_dir gt 90 then begin
           year_from_dir = year_from_dir + 1900
        endif else begin
           year_from_dir = year_from_dir + 2000
        endelse
        year = string(year_from_dir)
        if year_from_dir ne year_dir then begin
           message, /CONTINUE, 'WARNING: year in raw directory tree and filename do not agree.'
...
;        endif
        ...
        month_fits = months[fix(datearr[1])]
...
;        endif
        ...
        imonth = where(months eq month) + 1
...
;        endif
        ...
        if constructed_date_obs ne date_obs then begin
           message, /CONTINUE, 'Construction of DATE-OBS keyword from directory and filename information yielded ' + constructed_date_obs + ' which is different from FITS header value of ' + string(date_obs)
